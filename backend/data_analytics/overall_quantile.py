...
class OverallQuantileRegressor(PassengerQuantileRegressor):
    """Quantile Regression model for all train lines"""
    MIN = 5973
    ...
    def preprocess(self, df, rating_map=None):
        """Preprocess data for all train lines"""
        df = df.copy()
        ...
        df = df.groupby("Datetime").agg({
            ...
            "humidity": "mean",
            "pressure_mb": "mean",
            "Passenger_Rating": "first",
            "Passenger_Count": "sum",
        }).reset_index()
        df = df.drop(columns=["Datetime", "Rating_Label"], errors="ignore")
        if rating_map:
